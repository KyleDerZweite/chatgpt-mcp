...
// upload roots. On these platforms os.Root resolves each descendant relative
// to the held handle and rejects symlink traversal outside it. The standard
// library implementation uses component-by-component no-follow traversal where
...
func (roots *secureUploadRoots) openRelative(rootIndex int, relative string) (*os.File, error) {
	...
	return roots.roots[rootIndex].Open(relative)
}
